<?php include ('header.php'); ?>

<head>
    <meta charset="UTF-8">
    <meta name="viewport"
        content="width=device-width, initial-scale=1.0, user-scalable=1.0, minimum-scale=1.0, maximum-scale=1.0">
    <meta name='robots' content='index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1' />
    <title>Recipes</title>
    <meta name="description" content="description"
        content="Indian pesticides organic natural food farms/farming agriculture products certified cow milk, grains, pulses, oils, spices, frozen vegetables, produce near me" />
    <meta name="keywords"
        content="farm for vegetable, oil organic, pesticides organic food, organic products, organic agriculture, organic vegetables, organic cow milk, organic farms near me, natural farming, organic food farm near me">
    <meta property="og:locale" content="en_US" />
    <meta property="og:type" content="article" />
    <meta property="og:url" content="https://www.sanjeevaniagrofoods.com" />
    <meta property="og:site_name" content="Sanjeevani Agrofoods" />
    <meta property="article:publisher" content="https://www.facebook.com/profile.php?id=61556094441349" />
    <meta property="og:image"
        content="https://www.sanjeevaniagrofoods.com/beta/blog/uploads/post/sanjeevani-agrofoods.webp" />
    <meta property="og:image:height" content="1662" />
    <meta property="og:image:type" content="images/logo.webp" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:site" content="@SAgrofoods" />
    <meta name="twitter:label1" content="Est. reading time" />
    <meta name="twitter:data1" content="9 minutes" />

</head>

<body>








    <!----------------------------------------------------------------------------------------
                                         START PRICE SECTION 
    ------------------------------------------------------------------------------------------>
    <section>
        <div class="recipesSection mt-2">
            <img src="" alt="images">
        </div>
    </section>

    <section class="rv-20-price_section">

        <div class="container-fulid">
            <div class="recipesHeading">
                <div class="recipesIcon">
                    <svg fill="#def0a6" height="80px" width="80px" version="1.1" id="Capa_1"
                        xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 196.979 196.979" xml:space="preserve" stroke="#def0a6">
                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                            <g>
                                <path
                                    d="M43.48,0C28.1,0.003,15.588,12.517,15.588,27.896v141.188c0,15.38,12.512,27.894,27.893,27.896h131.66 c3.452,0,6.25-2.798,6.25-6.25v-21.646v-21.646V49.542V6.25c0-3.452-2.798-6.25-6.25-6.25H43.48z M43.481,12.5h125.41v37.042 v91.645H43.48c-5.687,0.001-10.978,1.716-15.392,4.649V27.896C28.088,19.408,34.994,12.501,43.481,12.5z M168.891,184.479H43.482 c-8.488-0.001-15.394-6.908-15.394-15.396c0-8.488,6.905-15.395,15.393-15.396h125.41v9.146h-98.41c-3.452,0-6.25,2.798-6.25,6.25 c0,3.452,2.798,6.25,6.25,6.25h98.41V184.479z">
                                </path>
                                <path
                                    d="M73.496,87.7v23.664c0,3.452,2.798,6.25,6.25,6.25h37.486c3.452,0,6.25-2.798,6.25-6.25V87.701 c8.258-3.315,13.982-11.413,13.982-20.669c0-11.935-9.437-21.706-21.24-22.244c-4.161-5.506-10.644-8.791-17.734-8.791 c-7.093,0-13.575,3.285-17.735,8.791c-11.804,0.538-21.241,10.31-21.241,22.244C59.514,76.289,65.239,84.386,73.496,87.7z M81.782,57.265c0.473,0,0.954,0.035,1.433,0.105c2.686,0.397,5.313-0.986,6.512-3.418c1.661-3.364,5.02-5.455,8.765-5.455 c3.744,0,7.103,2.091,8.766,5.457c1.2,2.432,3.839,3.811,6.513,3.416c0.476-0.07,0.956-0.105,1.429-0.105 c5.386,0,9.768,4.382,9.768,9.768c0,4.905-3.663,9.069-8.52,9.685c-3.122,0.396-5.463,3.053-5.463,6.2v22.196H85.996V83.331 c0-0.007,0.001-0.015,0.001-0.021c0.207-3.293-2.182-6.18-5.455-6.593c-4.862-0.613-8.528-4.776-8.528-9.685 C72.014,61.646,76.396,57.265,81.782,57.265z">
                                </path>
                            </g>
                        </g>
                    </svg>
                </div>
                <h3> Recipes Details</h3>
            </div>
        </div>
    </section>

    <div class="container">
        <ul class="nav nav-fill nav-tabs" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link active" id="fill-tab-0" data-bs-toggle="tab" href="#fill-tabpanel-0" role="tab"
                    aria-controls="fill-tabpanel-0" aria-selected="true"> Hindi </a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link" id="fill-tab-1" data-bs-toggle="tab" href="#fill-tabpanel-1" role="tab"
                    aria-controls="fill-tabpanel-1" aria-selected="false"> English </a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link" id="fill-tab-2" data-bs-toggle="tab" href="#fill-tabpanel-2" role="tab"
                    aria-controls="fill-tabpanel-2" aria-selected="false"> Panjabi </a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link" id="fill-tab-3" data-bs-toggle="tab" href="#fill-tabpanel-2" role="tab"
                    aria-controls="fill-tabpanel-3" aria-selected="false"> Urdu </a>
            </li>
        </ul>
        <div class="tab-content pt-5" id="tab-content">
            <div class="tab-pane active" id="fill-tabpanel-0" role="tabpanel" aria-labelledby="fill-tab-0">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Molestias sequi consectetur ipsum quas, quae rem, perspiciatis voluptates, alias tenetur quidem veniam? Quisquam voluptates dolore eveniet ad repudiandae facere alias eaque vero, neque delectus, hic doloremque ipsum quam modi porro sapiente magnam rerum rem accusantium quaerat! Deserunt voluptatibus a rerum quia?
            </div>
            <div class="tab-pane" id="fill-tabpanel-1" role="tabpanel" aria-labelledby="fill-tab-1">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aspernatur, sunt saepe? Quia quibusdam id quasi, fugiat totam consectetur quis veritatis. Voluptate sed harum quasi modi quod impedit rem optio deleniti iusto architecto quibusdam, possimus saepe voluptas aliquid ea. Saepe eligendi enim blanditiis optio aliquid, ipsa id illo est dolorum voluptatem.
            </div>
            <div class="tab-pane" id="fill-tabpanel-2" role="tabpanel" aria-labelledby="fill-tab-2">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aspernatur, sunt saepe? Quia quibusdam id quasi, fugiat totam consectetur quis veritatis. Voluptate sed harum quasi modi quod impedit rem optio deleniti iusto architecto quibusdam, possimus saepe voluptas aliquid ea. Saepe eligendi enim blanditiis optio aliquid, ipsa id illo est dolorum voluptatem.
            </div>
            <div class="tab-pane" id="fill-tabpanel-3" role="tabpanel" aria-labelledby="fill-tab-3">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aspernatur, sunt saepe? Quia quibusdam id quasi, fugiat totam consectetur quis veritatis. Voluptate sed harum quasi modi quod impedit rem optio deleniti iusto architecto quibusdam, possimus saepe voluptas aliquid ea. Saepe eligendi enim blanditiis optio aliquid, ipsa id illo est dolorum voluptatem.
            </div>
        </div>
    </div>


    <!----------------------------------------------------------------------------------------
                                         START PRICE SECTION 


    ------------------------------------------------------------------------------------------>

    <?php include ('footer.php'); ?>